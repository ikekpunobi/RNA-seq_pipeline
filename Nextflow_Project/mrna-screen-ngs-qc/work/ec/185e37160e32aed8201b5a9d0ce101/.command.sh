...
download() {
  ...
  curl -fL -C -         --retry 10 --retry-all-errors --retry-delay 10         --speed-time 60 --speed-limit 1024         "$url" -o "$out"
}
...
download "$url1" "ERR1860765_1.fastq.gz"

if [ "$layout" = "PAIRED" ]; then
  url2="https://$fq2"
  echo "Downloading $url2" >&2
  download "$url2" "ERR1860765_2.fastq.gz"
else
